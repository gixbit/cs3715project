...
lPagePopulator = function(data) {
	...
	aProfile.innerHTML = '<a href="/profile/'+ web +'">Profile</a>';
	aLocation.innerHTML = '<a class="active" href="/location/'+ web +'">Location</a>';
...
function getGpsLoc(callback){
	if (navigator.geolocation){
		navigator.geolocation.getCurrentPosition(callback, function(){},{enableHighAccuracy: true, maximumAge: 3000});
	}
	else {
...
function centerOn(p) {
	map.setCenter(marker[WebToKey[p]].position);
	map.setZoom(14);
	for (var j in marker) {
		try{
			if(j != WebToKey[p]){
